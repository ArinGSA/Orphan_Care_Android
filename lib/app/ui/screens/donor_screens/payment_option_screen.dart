...
class _PaymentOptionScreenState extends State<PaymentOptionScreen> {
  ...
  putDonorDonationData() async {
    ...
    if (commonController.checkKeyContainsInDataBase(Constants.orphanDataBase,
        "${donorController.donorSignUPModel.phoneNumber}-${Constants.donorDonationHistoryKey}-${trustData.phoneNumber}")) {
      list = commonController.getDataBaseByKey(Constants.orphanDataBase,
          "${donorController.donorSignUPModel.phoneNumber}-${Constants.donorDonationHistoryKey}-${trustData.phoneNumber}");
    }
    var donorDonationData = DonorDonationModel(
      ...
      location: trustData.city,
    );

    list.add(donorDonationData);

    await commonController.putDataBaseByKey(list, Constants.orphanDataBase,
        "${donorController.donorSignUPModel.phoneNumber}-${Constants.donorDonationHistoryKey}-${trustData.phoneNumber}");
  }
  ...
  putTrustDonationData() async {
    ...
    if (commonController.checkKeyContainsInDataBase(Constants.orphanDataBase,
        "${trustData.phoneNumber}-${Constants.trustDonationHistoryKey}")) {
      list = commonController.getDataBaseByKey(Constants.orphanDataBase,
          "${trustData.phoneNumber}-${Constants.trustDonationHistoryKey}");
    }
    ...
    print(donorController.donorSignUPModel.donorName!);
    print(donorController.donorSignUPModel.phoneNumber!);
